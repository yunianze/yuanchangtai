...
class connector {
	...
	function admin_purview() {
		if ($this->fun->accept('archive', 'R') == 'filemanage' && $this->fun->accept('action', 'R') == 'batupfilesave') {
			$ecisp_admininfo = $this->fun->accept('ecisp_admininfo', 'C') ? $this->fun->accept('ecisp_admininfo', 'C') : $this->fun->accept('ecisp_admininfo', 'P');
			$esp_powerlist = $this->fun->accept('esp_powerlist', 'C') ? $this->fun->accept('esp_powerlist', 'C') : $this->fun->accept('esp_powerlist', 'P');
			$gettype = false;
		} else {
	...
	function member_cookieview($keyword = false) {
		...
		if (!preg_match("/^[^!@~`\'\"#\$\%\^&\*\(\)\+\-\{\}\[\]\|\\/\?\<\>\,\.\:\;]{2,30}$/i", $retrunstr['username']) && !empty($retrunstr['username'])) {
	...
	function get_payplug_array($opid = 0, $isclass = 1, $ismoblie = 2) {
		...
		$chacherray = $this->dbcache->checkcache('orderpay_array_' . $isclass . '_' . $ismoblie, false);
		$arrayList = array();
		if (!$chacherray) {
			$rs = $this->db->query($sql);
			while ($rsList = $this->db->fetch_assoc($rs)) {
			...
			$chacherray = $this->dbcache->cachesave('orderpay_array_' . $isclass . '_' . $ismoblie, $array);
			$chacherray = $chacherray ? $chacherray : $array;
			unset($array);
		}
	...
	function get_link($module = null, $read = array(), $lng_temp = '', $pagekey = 0, $patyclass = 0, $is_wap = 0) {
		...
		switch ($module) {
			...
			case 'wxposeturl':
				...
				break;
			case 'pclink':
				$link = "index.php?ac=scriptout&at=dsitelink&type=" . $read['type'];
				break;
		}
