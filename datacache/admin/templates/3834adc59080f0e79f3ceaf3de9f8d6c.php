...
<link href="templates/css/treeleftview.css" rel="stylesheet" type="text/css"/>
<script type="text/javascript" src="js/treeviewcookie.js" ></script>
<script type="text/javascript" src="js/treeleftview.js" ></script>
<script type="text/javascript">
	...
	$().ready(function() {
		...
		$('.divRight').css({width:w-175,height:h});$('#centerleft').css({height:h-10});$('#centerhidden').css({height:h});$('#srcolcontent').css({height:h-175});$('#modelleftcontent').css({height:h-45});$('#firefoxuser').html(($.browser.mozilla?'yes, your browser is Firefox':'oh, you are not using Firefox?'));$('#ieuser').html(($.browser.msie?'yes, your browser is Internet Explorer':'oh, you are not using Internet Explorer?'));<?php echo $this->_tpl_vars['jsstr'] ?>
		$("#clickmenubotton li span").click(function() {
		...
		$(".srcolcontent,#srcolcontent").jscroll({W:"15px",BgUrl:"url(templates/images/s_bg.gif)",Bg:"right 0 repeat-y",Bar:{Bd:{Out:"#bcbcbc",Hover:"#ccc"},Bg:{Out:"-45px 0 repeat-y",Hover:"-58px 0 repeat-y",Focus:"-71px 0 repeat-y"}},Btn:{btn:true,uBg:{Out:"0 0",Hover:"-15px 0",Focus:"-30px 0"},dBg:{Out:"0 -15px",Hover:"-15px -15px",Focus:"-30px -15px"}},Fn:function(){}});
		$("#centerhidden").toggle(function () {scrolclear();},function () {scrolopen();});
	});
	function scrolclear(){var w=$(window).width()-20;$("#centerleft").addClass("centerleftnone");$("#lefthidden").addClass("bottonlefthover2");$('.divRight').css({width:w})}function scrolopen(){var w=$(window).width()-20;var w2=w-175;$("#centerleft").removeClass("centerleftnone");$("#lefthidden").removeClass("bottonlefthover2");$('.divRight').css({width:w2})}function sizewindow(){var w=$(window).width()-20;var h=$(window).height()-98;var lefttype=$("#centerleft").css('display');if(lefttype=='none'){$('.divRight').css({width:w,height:h})}else{$('.divRight').css({width:w-175,height:h})}$('#centerleft').css({height:h-10});$('#centerhidden').css({height:h});$('#srcolcontent').css({height:h-175});$('#modelleftcontent').css({height:h-45});$('#jerichotab_contentholder').css({height:h-40});$('.jerichotabiframeclass').css({height:h-40})}
