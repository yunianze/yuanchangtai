...
class important extends connector {
	...
	function onpaysave() {
		...
		$ismoblie = intval($this->fun->accept('ismoblie', 'P'));
		$ismoblie = empty($ismoblie) ? 0 : $ismoblie;
		...
		$date = time();
		if ($inputclass == 'add') {
			$db_field = 'pid,payname,paycontent,paycode,payis,payplugver,pluglist,isclass,addtime,ismoblie';
			$db_values = "50,'$payname','$paycontent','$plugcode',$payis,'$version','$payconfig',0,$date,$ismoblie";
			...
			$db_where = 'opid=' . $opid;
			$db_set = "payname='$payname',paycontent='$paycontent',payis=$payis,pluglist='$payconfig',ismoblie=$ismoblie";
